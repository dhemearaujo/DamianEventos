...
class WCFMmp_Shipping_Zone {
  ...
  public static function delete_shipping_methods( $data ) {
    ...
    $result = $wpdb->query( $wpdb->prepare( "DELETE FROM {$table_name} WHERE zone_id=%d AND vendor_id=%d AND instance_id=%d", $data['zone_id'], apply_filters( 'wcfm_current_vendor_id', get_current_user_id() ), $data['instance_id'] ) );
  ...
  public static function update_shipping_method( $args ) {
    ...
    $data = array(
        ...
        'vendor_id' => empty( $args['vendor_id'] ) ? apply_filters( 'wcfm_current_vendor_id', get_current_user_id() ) : $args['vendor_id'],
  ...
  public static function toggle_shipping_method( $data ) {
    global $wpdb;
    $table_name = "{$wpdb->prefix}wcfm_marketplace_shipping_zone_methods";
    $updated    = $wpdb->update( $table_name, array( 'is_enabled' => $data['checked']  ), array( 'instance_id' => $data['instance_id' ], 'zone_id' => $data['zone_id'], 'vendor_id' => apply_filters( 'wcfm_current_vendor_id', get_current_user_id() ) ), array( '%d' ) );
