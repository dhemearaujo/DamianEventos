...
class WCFMmp_Shortcode {
	...
	public function wcfmmp_stores_map_shortcode( $atts ) {
		...
		$WCFMmp->template->get_template( 'shortcodes/wcfmmp-view-store-lists-map.php', $template_args );
		if( $filter ) {
			if( $search || $category || $country || $state ) { $WCFMmp->template->get_template( 'shortcodes/wcfmmp-view-store-lists-search-form.php', $template_args ); }
		}
		echo '</div>';
