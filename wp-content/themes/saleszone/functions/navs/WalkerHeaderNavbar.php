...
class WalkerHeaderNavbar extends Walker_Nav_Menu {
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if( $depth == 0 ) {
            $classes[] = 'pc-header-layout-3__navbar-item';
            $classes[] = ($item->current || $item->current_item_ancestor) ? 'pc-header-layout-3__navbar-item--active' : '';
        } else {
            $classes[] = 'overlay__item';
            $classes[] = ($item->current || $item->current_item_ancestor) ? 'overlay__item--active' : '';
        }
