...
class WCFMmp_Store {
	...
	public function get_store_tabs() {
		...
		if( apply_filters( 'wcfm_is_allow_store_articles', true ) ) {
	...
	public function get_followings_url() {
		return $this->get_shop_url() . 'followings';
	}
	...
	public function get_articles_url() {
		return $this->get_shop_url() . 'articles';
	}
		...
		if( !$vendor_tax_migrated || apply_filters( 'wcfmmp_force_store_taxonomy_refresh', false ) ) {
			...
			if( !empty( $vendor_products ) ) {
				foreach( $vendor_products  as $vendor_product_id => $vendor_product ) {
					$pcategories = get_the_terms( $vendor_product, 'product_cat' );
