...
<script type="text/template" id="wc_psearch_result_itemTpl"><span class="rs_rs_avatar"><a href="{{= url }}"><img src="{{= image_url }}" /></a></span>
	<div class="rs_content">
		...
		{{ if ( price != null && price != '' ) { }}<div class="rs_rs_price"><?php wc_ps_ict_t_e( 'Preço', __('Preço', 'woocommerce-predictive-search' ) ); ?>: {{= price }}</div>{{ } }}
		...
		{{ if ( categories.length > 0 ) { }}
			<div class="rs_rs_cat posted_in">
				<?php wc_ps_ict_t_e( 'Categoria', __('Categoria', 'woocommerce-predictive-search' ) ); ?>: 
				{{ var number_cat = 0; }}
