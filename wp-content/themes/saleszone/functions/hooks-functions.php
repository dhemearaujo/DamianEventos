...
if ( !function_exists( 'saleszone_render_loop_product_image' ) ) {
    function saleszone_render_loop_product_image()
    {
        ...
        if ( saleszone_option( 'category-product-flipper' ) && $flip_image_src ) {
...
if ( !function_exists( 'saleszone_render_saleszone_search' ) ) {
    function saleszone_render_saleszone_search()
    {
        if ( saleszone_is_plugin_active( 'premmerce-search/premmerce-search.php' ) ) {
            get_template_part( 'parts/forms/autocomplete' );
        }
    }
        ...
        if ( function_exists( 'premmerce_wishlist' ) ) {
            ...
            <div class="product-snippet__action-item">
                <div class="pc-product-action"
                     data-ajax-inject="wishlist-button--<?php 
            echo  esc_attr( $product->get_ID() ) ;
            ?>"> <?php 
            wc_get_template( '../premmerce-woocommerce-wishlist/wishlist-btn.php', array(
                'type' => 'link',
...
if ( !function_exists( 'saleszone_render_wishlist_delete_buttom' ) ) {
    function saleszone_render_wishlist_delete_buttom()
    {
        ...
        if ( function_exists( 'premmerce_wishlist' ) ) {
            ...
            if ( isset( $wishlistPage ) && $wishlistPage == true ) {
                ...
                $deleteUrl = wp_nonce_url( $apiUrl . $wishlist_current['wishlist_key'] . '/' . $product->get_ID(), 'wp_rest' );
                wc_get_template( '../premmerce-woocommerce-wishlist/wishlist-btn-delete.php', array(
                    'deleteUrl' => $deleteUrl,
                ) );
            }
...
if ( !function_exists( 'saleszone_render_wishlist_move_button' ) ) {
    function saleszone_render_wishlist_move_button()
    {
        
        if ( function_exists( 'premmerce_wishlist' ) ) {
            global  $wishlistPage ;
            if ( isset( $wishlistPage ) && $wishlistPage == true && count( premmerce_wishlist()->getWishlists() ) > 1 ) {
                wc_get_template( '../premmerce-woocommerce-wishlist/wishlist-btn-move.php', array() );
            }
        }
...
if ( !function_exists( 'saleszone_render_compare_catalog_button' ) ) {
    function saleszone_render_compare_catalog_button()
    {
        ...
        global  $product ;
        $url = site_url( 'wp-json/premmerce/comparison/add' );
